...
async function run () {
  ...
  const updatedEquipment = await client.put(`/equipments/${equipment.id}`, {
    name: 'Air conditioner Split with photo',
    archived: false,
    notes: 'Equipment registered through the collaborator app (updated)',
    number: '1997',
  ...
  assert.equal(updatedEquipment.name, 'Air conditioner Split with photo')
}
